s = """F|--L7J.L7|7|-LF7-7--7F7--7F7J--F7J.7F|J7.F7|.-FJF|J.F7F--LFF.7.F7JFL-JFJ7F|.FL77-F7-J.F7-F7L-FF-F-L7F-|7.FL7-7F|7F7.F-F|-77FJ7FF-J777.|FFFL
...
vy = [0,1,0,-1]
vx = [-1,0,1,0]
m = {"|":[1,0,1,0],"-":[0,1,0,1],"L":[0,0,1,1],"J":[0,1,1,0],"7":[1,1,0,0],"F":[1,0,0,1], "S":[1,1,1,1]}
areas = {"|":1,"-":1,"L":0.5,"J":0.5,"7":0.5,"F":0.5, "S":0}
...
while f:
    # print(xx,yy)
    f = False
    for index, (vyy, vxx) in enumerate(zip(vy, vx)):
        nx, ny = xx+vxx,yy+vyy
        if nx < 0 or nx >= x or ny < 0 or ny >= y:
            continue
        if m.get(lines[xx][yy], [0,0,0,0])[(index+2)%4] == 0:
            continue
        if m.get(lines[nx][ny], [0,0,0,0])[index] == 0:
            continue
        if (nx,ny) in points:
            continue
        points.append((nx,ny))
...
print(points)
print([points[i][0]*points[i+1][1]-points[i][1]*points[i+1][0] for i in range(len(points)-1)])
print(abs(sum(points[i][0]*points[i+1][1]-points[i][1]*points[i+1][0] for i in range(len(points)-1))/2)-l)

# didn't realize raying actually could work
# spent a whole hour reasoning about how to apply shoelace formula
...
#         if contained:
#             ans += len(area)
#             print("found", len(area))
#         print(sorted(list(area)))
