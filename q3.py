s = """......644............612.......254..638..............802.................................118.....................................317.691....
...
for i in range(len(lines)):
    ...
    for j in range(len(lines[0])):
        if lines[i][j] in "1234567890":
            s = s*10+int(lines[i][j])
        else:
            if s != 0:
                # print(arr[max(0,i-len(str(s))-1):i+1,max(0,j-1):j+2])
                if np.sum(arr[max(0,i-1):i+2,max(0,j-len(str(s))-1):j+1]) > 0:
                    ans += s
                    # print(s)
                s = 0
    if np.sum(arr[max(0,i-1):i+2,max(0,j-len(str(s))-1):j+1]) > 0:
        ...
        if lines[i][j] in "1234567890":
            ...
            if s != 0:
                for ii in range(max(0,i-1), min(i+2, len(lines))):
                    for jj in range(max(0,j-len(str(s))-1), min(j+1,len(lines[0]))):
                        if lines[ii][jj] == "*":
                            if (ii,jj) not in gears:
                                gears[(ii,jj)] = []
                            gears[(ii,jj)].append(s)
                s = 0
    if s != 0:
        for ii in range(max(0,i-1), min(i+2, len(lines))):
            for jj in range(max(0,j-len(str(s))-1), min(j+1,len(lines[0]))):
...
ans = 0
for arr in gears.values():
    if len(arr) != 2:
        continue
    ans += arr[0] * arr[1]
print(ans)

# very ugly but works. don't have energy to optimize