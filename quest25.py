...
for start in n:
    ...
    for cur in q:
        for next in g[cur]:
            ...
            visited.add(next)
            q.append(next)
            cnt[(min(cur, next), max(cur,next))] += 1

l = list(cnt.items())
l = sorted(l, key=lambda x: -x[1])
print(l)

# l = [(('jqt', 'nvd'), 13), (('bvb', 'cmg'), 13), (('hfx', 'pzl'), 13)]
...
# print(visited)
# print(n)
print(len(visited), len(n), len(visited) * (len(n) - len(visited)))

# a very enjoying one. Merry Christmas!
